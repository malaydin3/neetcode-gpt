...
class MultiHeadedSelfAttention(nn.Module):
    ...
    def __init__(self, embedding_dim: int, attention_dim: int, num_heads: int):
        ...
        for i in range(num_heads):
            self.att_heads.append(self.SingleHeadAttention(embedding_dim, self.head_size))

        self.output_projection = nn.Linear(self.attention_dim, self.attention_dim, bias = False)
    ...
    def forward(self, embedded: TensorType[float]) -> TensorType[float]:
        ...
        for head in self.att_heads:
            head_outputs.append(head(embedded))  # Pass full embedding to each head
        concatenated = torch.cat(head_outputs, dim=2)  # Concat along feature dim
        return torch.round(self.output_projection(concatenated), decimals=4)
